...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
	// optimization: make only a single call to document.querySelectorAll().
	// previously, making several queries on the DOM was unnecessary and was taking a significant amount of time.
	// now, testing has this clocking in at 1.209000000017113ms
...
function updatePositions() {
  ...
  // optimization: calculate document.body.scrollTop/1250 once (outside loop) instead of many times (inside loop)
  var scrollTopFraction = (document.body.scrollTop / 1250);
  var phaseTable = new Array(Math.sin(scrollTopFraction + 0), Math.sin(scrollTopFraction + 1), Math.sin(scrollTopFraction + 2), Math.sin(scrollTopFraction + 3), Math.sin(scrollTopFraction + 4));
  for (var i = 0; i < items.length; i++) {
    var phase = phaseTable[i % 5];
